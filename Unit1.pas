...
type
  ...
  TVertice=class
    ...
    ID:        word;                   //Vertice №
    constructor Create;
    procedure Equalize (out VerticeOut: TVertice);
  end;
  ...
  TEdge = class
    ...
    IISrc:     byte;    //independent source. produces: 0=none, 1=current
    IISPhase:  real;    //IS's phase, degrees
    IISAmp:    real;    //IS's amplitude
    IUSrc:     byte;    //independent source. produces: 0=none, 1=voltage
...
//    Tree:         TGraph;
    ...
    procedure EnumEdges;
    procedure NewZYMatrix (h: real); //replace common Z, Y matrix with  special synth one
    function SolveSynth: Matrix;
    ...
    function SolveMKT (p:integer=0): Matrix;
    procedure Restore;
    procedure DeleteEdge (EdgeNum: word);
  ...
  const deltaT=0.0001;      //dt for synth method (a.k.a "h")
        FullT=0.05; //full time for integral
var
  resultUC:array[1..24,0..80000] of real;  //1..12: I; 13..24: U
  GraphSet: boolean;
  locale: string;
  Form1: TForm1;
  ...
  Z_external, E_external1, E_external2: Matrix;
implementation
...
procedure TGraph.NewZYMatrix(h: Real);
var a, b: integer;
R, L, C: real;
begin
  for a:=1 to EdgeCount do begin
    R:=Edges[a].eR;
    L:=Edges[a].eL/1000;
    C:=Edges[a].eC/1000000;
    if (Edges[a].eL>0) or (Edges[a].eC>0) then Edges[a].IISrc:=1;
    Edges[a].IISPhase:=Edges[a].eL;         //this is maxximum bull shit
    Edges[a].IUSPhase:=Edges[a].eC;
    Edges[a].eL:=0;
    Edges[a].eC:=0;
    if L>0 then
      MY.Comp[a,a].Re:=h/L;
    if C>0 then
      MY.Comp[a,a].Re:=C/h;
    MZ.Comp[a,a].Re:=1/MY.Comp[a,a].Re;
    Edges[a].eR:=MZ.Comp[a,a].Re;
  end;
  DefineVertices;
  DrawGraph;
end;
...
function TGraph.SolveSynth: Matrix;
var
a, b, c, ticks, rec: integer;
h, T: real;  //h = "dt", T = full integral time
StepUI, StepUI2, StepJ, cacheZ, cacheY: Matrix; //stores each step's UC and IL for further usage

begin
  CreateMatrix;

  Z_external:=MatrixMult(MCyc, MZ);      //BZ
  Z_external:=MatrixMult(Z_external, transpose(MCyc));  //BZBt = Zk
  Z_external:=MatrixInverse(Z_external);

  E_external1:=MatrixMult(MCyc, ME);
  E_external2:=MatrixMult(MCyc, MZ);

  StepUI:=SolveMKT;
  Form1.PrintMatrix(StepUI);
  for a:=0 to 100 do
    for rec:=1 to 12 do begin
      ResultUC[rec,a]:=StepUI.Comp[rec,1].Re;
      ResultUC[rec+12,a]:=StepUI.Comp[rec,2].Re;
    end;
//  Result:=SolveMKT(1);
  NewZYMatrix(deltaT);
  cacheZ:=MZ;
  ...
  StepJ:=MJ;
  for a:=1 to EdgeCount do begin
    if Edges[a].IUSPhase>0 then begin //determining actual U_C's
      //StepUI.Comp[a,2].Re:=StepUI.Comp[a,2].Re-Edges[a].eR*StepUI.Comp[a,1].Re;  //deltaU - IR = U_C
      StepJ.Comp[1,a].Re:=Edges[a].IUSPhase/1000000*StepUI.Comp[a,2].Re/deltaT;
    end;
    if Edges[a].IISPhase>0 then
      StepJ.Comp[1,a].Re:=-StepUI.Comp[a,1].Re;
    if StepJ.Comp[1,a].Re<>0 then
      Edges[a].IISrc:=1;

    Edges[a].IISAmp:=StepJ.Comp[1,a].Re;
    MJ:=StepJ;
  end;
  //ME:=StepJ;
  ...
  Edges[1].eR:=1;
  CreateMatrix;
  cacheZ.Comp[1,1].Re:=1;
  cacheY.Comp[1,1].Re:=1;
  ...
  MZ:=cacheZ;
  MY:=cacheY;
  ticks:=round(FullT/deltaT);  //part 2. iterations
  for b:=100 to 300 do begin
    form1.Button20.Caption:=inttostr(b);
    StepUI2:=SolveMKT;       //taking previously recorded IISAmp's aswell
    //StepJ:=MJ;
    for a:=1 to EdgeCount do begin
      if Edges[a].IUSPhase>0 then begin       //.eC>0
        //StepUI2.Comp[a,2].Re:=StepUI2.Comp[a,2].Re-Edges[a].eR*StepUI2.Comp[a,1].Re;
        MJ.Comp[1,a].Re:=Edges[a].IUSPhase/1000000*StepUI2.Comp[a,2].Re/deltaT;  //J = -C/h*U_C(n)
      end;
      if Edges[a].IISPhase>0 then          //.eL>0
        MJ.Comp[1,a].Re:=-StepUI2.Comp[a,1].Re;
      Edges[a].IISAmp:=MJ.Comp[1,a].Re;
    end;
    //MJ:=StepJ;
    for rec:=1 to 12 do begin
      ResultUC[rec,b]:=StepUI2.Comp[rec,1].Re;
      ResultUC[rec+12,b]:=StepUI2.Comp[rec,2].Re;
    end;

//    ResultUC[1,b]:=StepUI2.Comp[6,2].Re;
  end;
  Result:=SolveMKT;
end;

procedure TVertice.Equalize(out VerticeOut: TVertice);
begin

end;

procedure TGraph.DeleteEdge(EdgeNum: word);
var a: integer;
cacheEdge: TEdge;
cacheVertice: TVertice;
begin
  for a:=EdgeNum+1 to EdgeCount do begin
    dec(Edges[a].ID);
    Edges[a].Equalize(Edges[a-1]);
  end;
  dec(EdgeCount);
  DefineVertices;
end;
...
function TGraph.SolveMKT (p:integer=0): Matrix;
var a,b,c: integer;
Zk, Ik, Ek, Ek1, Ek2, U: Matrix;
GraphCache: TGraph;
begin
...
//  Zk:=MatrixInverse(Zk);
 ...
 // Ek2:=MatrixMult(MCyc, MZ);
  ...
  Ek2:=MatrixMult(E_external2, MJ);
  Ek:=MatrixSub(E_external1, Ek2);
  Result:=MatrixMult(Z_external, Ek);
  Result:=MatrixMult(transpose(Result),MCyc);
  U:=MatrixAdd(transpose(Result),MJ);     //  I+J
  U:=MatrixMult(MZ, U);     //            Z(I+J)
  U:=MatrixSub(U, ME);      //            Z(I+J)-E
  Result.Y:=2;
  U:=transpose(U);
  for a:=1 to EdgeCount do begin
    //Result.Body[a,2]:=U.Body[a,1];
    Result.Comp[a,2]:=U.Comp[a,1];
  end;
  //Form1.PrintMatrix(Ik);
  //Result:=Ik;
end;
...
begin
  if not Res then begin
    ...
    StringGrid1.Cells[0,1]:='I, A';
    StringGrid1.Cells[0,2]:='U, V';
    StringGrid1.Cells[0,0]:='Edge №';
  end;
  ...
  if Res then begin
    ...
    for a:=1 to M.X do begin
      for b:=1 to M.Y do begin
        StringGrid2.Cells[a,b]:=ComplString(M.Comp[a,b]);
        StringGrid2.Cells[a,0]:=inttostr(a);
       end;
    end;
    StringGrid2.Cells[0,1]:='I, A';
    StringGrid2.Cells[0,2]:='U, V';
...
//incidence
  ...
  for a:=1 to EdgeCount do begin
    ...
    if Edges[a].eL>0 then ReZ:=ReZ+1/inf;//ImZ:=ImZ+(Edges[a].eL*Omega)/1000;
    if Edges[a].eC>0 then ReZ:=inf;//ImZ:=ImZ-1000000/(Omega*Edges[a].eC);
...
//      ImZ:=inf;
    ...
    MZ.Comp[a,a]:=Compl(ReZ, ImZ);
     if (ReZ=0) and (ImZ=0) then begin
      MY.Comp[a,a]:=Compl(inf,0);
      end else begin
        MY.Comp[a,a]:=ComplDiv(Compl(1,0), Compl(ReZ, ImZ));
      end;
    if (Edges[a].IISrc=1) and (ReZ=0) and (ImZ=0) then begin
      MZ.Comp[a,a]:=Compl(inf,0);
    ...
    if Edges[a].IUSrc=1 then begin
      ...
      if (ReZ=0)and(ImZ=0) then
        MZ.Comp[a,a]:=Compl(1/inf,0);
    end;
    if not Edges[a].Enabled then begin
      MY.Comp[a,a]:=Compl(1/inf,0);
      ME.Comp[1,a]:=Compl(0,0);
      MJ.Comp[1,a]:=Compl(0,0);
    end;
  end;
end;
...
begin
  ...
  if Edges[EdgeNum].IsTree then begin
    ...
    initEnableState:=Edges[EdgeNum].Enabled;
    Edges[EdgeNum].Enabled:=false;
    ...
    Result:=CutArr;
    Edges[EdgeNum].Enabled:=initEnableState;
  end;
end;
...
begin
  ...
  for a:=1 to EdgesN do begin   //searching for every vertice/edge connected to input vertice
   LnID:=Vertices[Vertice].LineID[a];
   if Edges[LnID].Enabled then begin
    if Edges[LnID].side1=Vertice then
      VertID:=Edges[LnID].side2 else
        VertID:=Edges[LnID].side1;    //here we found another Vertice ID!
    Wood:=false;
...
begin
  ...
  Form1.Image1.Canvas.TextOut(round(Center.X / 0.6), round (Center.Y / 0.53), 'Frequency: '+inttostr(Graph.Freq)+' Hz');
end;
...
begin
  ...
  Graph.VerticeXY[9].X:=round(Center.X / 1);
  Graph.VerticeXY[9].Y:=round(Center.Y / 0.7);
  Graph.VerticeXY[10].X:=round(Center.X / 1);
  Graph.VerticeXY[10].Y:=round(Center.Y / 1.6);
  Graph.VerticeXY[11].X:=round(Center.X / 0.7);
  Graph.VerticeXY[11].Y:=round(Center.Y / 1);
  Graph.VerticeXY[8].X:=round(Center.X / 2);
  Graph.VerticeXY[8].Y:=round(Center.Y / 0.7);
...
var a,b: integer;
begin
  PrintMatrix (Graph.SolveCut, true);
end;
...
begin
  Form4.show;
  Form4.Image2.Picture.LoadFromFile('img/legend.jpg');
end;
...
begin
  ...
  PrintMatrix(Graph.SolveSynth, true);
end;
...
begin
  PrintMatrix (Graph.SolveNVM, true);
end;

procedure TForm1.Button11Click(Sender: TObject);
begin
  PrintMatrix(Graph.SolveMesh, true);
end;
